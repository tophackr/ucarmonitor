import type { ApiTags, TagDescription } from '../types/ApiTags'

interface TagsToListProps<T extends ApiTags, R extends { id: string }> {
    tag: T
    result?: R | R[]
    depsTags?: T[]
}

export function invalidatesTagsToList<
    T extends ApiTags,
    R extends { id: string }
>({ tag, result, depsTags }: TagsToListProps<T, R>): TagDescription<T>[] {
    const tagList: TagDescription<T>[] = [{ type: tag, id: 'LIST' }]

    if (depsTags)
        tagList.concat(depsTags.map(tag => ({ type: tag, id: 'LIST' })))
    if (!result) return tagList

    return Array.isArray(result)
        ? tagList.concat(result.map(({ id }) => ({ type: tag, id })))
        : tagList.concat({ type: tag, id: result.id })
}

export function providesTagsToList<
    T extends ApiTags,
    R extends { id: string }
>({ tag, result, depsTags }: TagsToListProps<T, R>): TagDescription<T>[] {
    if (!result) return []

    if (Array.isArray(result)) {
        const tagList: TagDescription<T>[] = [{ type: tag, id: 'LIST' }]

        if (depsTags)
            tagList.concat(depsTags.map(tag => ({ type: tag, id: 'LIST' })))

        tagList.concat(result.map(({ id }) => ({ type: tag, id })))

        return tagList
    } else {
        return [{ type: tag, id: result.id }]
    }
}
