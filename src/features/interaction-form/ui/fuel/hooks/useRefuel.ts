...
export function useRefuel(fuelCapacity?: number | null): UseRefuelReturn {
    ...
    const { setValue, watch } = useFormContext<FuelInteractionData>()
    ...
    const onBeforeChange = useCallback(
        (value: number) => {
            setValue('data.beforeRefueling', getCapacity(value, fuelCapacity))
        },
    ...
    const onAfterChange = useCallback(
        (value: number) => {
            setValue('data.afterRefueling', getCapacity(value, fuelCapacity))
        },
        [fuelCapacity, setValue]
    )
    ...
    const onFullChange = useCallback(
        (value: boolean) => {
            if (!value) {
                setValue('data.afterRefueling', fuelCapacity)
            }
        },
        [fuelCapacity, setValue]
    )
